...
public class ItemGenericSlab extends ItemBlock {
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ) {
        if (stack.stackSize == 0)
            ...
            if (block == field_150939_a && (side == 1 && !flag || side == 0 && flag)) {
                ...
                return func_150946_a(stack, world, x, y, z, side) || super.onItemUse(stack, player, world, x, y, z, side, hitX, hitY, hitZ);
        }
    }
    ...
    public boolean func_150936_a(World world, int x, int y, int z, int side, EntityPlayer player, ItemStack stack) {
        ...
        if (block == field_150939_a && meta == 2)
            ...
            return block1 == field_150939_a || super.func_150936_a(world, i1, j1, k1, side, player, stack);
        }
    }

    private boolean func_150946_a(ItemStack stack, World world, int x, int y, int z, int side) {
        if (side == 0)
            y--;
        if (side == 1)
            y++;
